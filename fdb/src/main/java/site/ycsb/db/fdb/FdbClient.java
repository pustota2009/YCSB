 ...
 * ied. See the License for the specific language governing
...
import com.light.fdb.*;
import com.light.fdb.client.*;
import com.light.fdb.client.factories.*;
import com.light.fdb.factory.ObjectFactory;
import com.light.fdb.client.factories.FlashDbNioClientObjectFactory;
import com.light.fdb.util.Bytes;
import com.light.fdb.util.UniformSplit;
import com.light.shaded.com.google.common.primitives.Longs;
...
public class FdbClient extends site.ycsb.DB {
  ...
  //private static ObjectFactory objectFactory = new FlashDbNioClientObjectFactory();
  //private static ObjectFactory objectFactory = new FlashDbObjectFactory();
  ...
  private int tableRegionCount;
  private int batchSize;
  private boolean connectionNIO;
    ...
    try {
      ...
      synchronized (THREAD_COUNT) {
        if (connection == null) {
          ...
          if (connectionNIO) {
            System.out.println("Enabled NIO mode");
            ObjectFactory objectFactory = new FlashDbNioClientObjectFactory();
            config = objectFactory.configuration();
            config.addResource(new File("fdb-site.xml"));
            connection = objectFactory.connection(config);
          } else {
            System.out.println("Enabled Local Thread mode");
            config = Configuration.create();
            config.addResource(new File("fdb-site.xml"));
            connection = ConnectionFactoryThreadLocal.createConnection(config);
          }
...
/*      if (currentTable != null) {
  ...
  public void getTable(String namespace, String table) throws IOException {
    final TableName tName = TableName.valueOf(namespace, table);
    this.currentTable = (Table) connection.getTable(tName);
  }
  ...
  public Status read(String table, String key, Set<String> fields, Map<String, ByteIterator> result) {
    ...
    try {
      for (int i = 0; i < batchSize; i++) {
        while (r[i].cellScanner().advance()) {
          final Cell c = r[i].current();
  ...
  public Status delete(String table, String key) {
    ...
    final Delete d = new Delete(makeHbaseRowKey(key));
    try {
