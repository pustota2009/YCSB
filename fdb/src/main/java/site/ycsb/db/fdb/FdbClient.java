...
import com.light.fdb.TableNameImpl;
import com.light.fdb.api.*;
import com.light.fdb.api.CellScanner;
import com.light.fdb.api.Connection;
import com.light.fdb.client.*;
import com.light.fdb.config.FdbConfiguration;
...
public class FdbClient extends site.ycsb.DB {
  ...
  private TableImpl currentTable = null;
...
//    }
    ...
    batchSize =
        Integer.parseInt(getProperties().getProperty(BATCH_SIZE, BATCH_SIZE_DEFAULT));

    tableRegionCount =
        Integer.parseInt(getProperties().getProperty(TABLE_REGION_COUNT, TABLE_REGION_COUNT_DEFAULT));
    ...
    try {
      ...
      synchronized (THREAD_COUNT) {
        if (connection == null) {
          ...
          AdminImpl adm = (AdminImpl)connection.getAdmin();
          ...
          // starts.
          final TableName tName = TableNameImpl.valueOf(namespace, table);
...
/*      if (currentTable != null) {
  ...
  public void getTable(String namespace, String table) throws IOException {
    final TableName tName = TableNameImpl.valueOf(namespace, table);
    this.currentTable = (TableImpl) connection.getTable(tName);
  }
  ...
  public Status read(String table, String key, Set<String> fields, Map<String, ByteIterator> result) {
    ...
    try {
      ...
      for (int i = 0; i < batchSize; i++) {
        Get g = new GetImpl(makeHbaseRowKey(key + "_" + i));
  ...
  public Status scan(String table, String startkey, int recordcount,
    ...
    Get g = new GetImpl(makeHbaseRowKey(startkey));
  ...
  public Status update(String table, String key, Map<String, ByteIterator> values) {
    ...
    if (!tableName.equals(table)) {
      ...
      try {
        ...
        TableName tName = TableNameImpl.valueOf(namespace, table);
        if (!adm.tableExists(tName)) {
          adm.createTable(tName, new UniformSplit().split(tableRegionCount));
        }
    ...
    for (int i = 0; i < batchSize; i++) {
      Put p = new PutImpl(makeHbaseRowKey(key + "_" + i));
      for (byte[] k : hmQV.keySet()) {
        p.addColumn(columnFamilyBytes, k, hmQV.get(k));
      }
      lstPut.add(p);
    }
  ...
  public Status delete(String table, String key) {
    ...
    final Delete d = new DeleteImpl(makeHbaseRowKey(key));
    try {
